...
class AlbertModelMU(AlbertPreTrainedModel):
...
class AlbertForMaskedLMMU(AlbertPreTrainedModel):
    ...
    def forward(
        ...
        if remove_bias:
            # sequence_output = torch.nn.functional.normalize(sequence_output, p=2, dim=-1)
            sequence_outputs = self.filter_model.encode(sequence_outputs)  # filter_model
            # sequence_output = torch.nn.functional.normalize(sequence_output, p=2, dim=-1)
...
class AlbertForSequenceClassificationMU(AlbertPreTrainedModel):
    def __init__(self, config: AlbertConfig):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
    ...
    def forward(
        ...
        if remove_bias:
            ...
            pooled_output = self.filter_model.encode(pooled_output)  # filter_model
            # pooled_output = torch.nn.functional.normalize(pooled_output, p=2, dim=-1)
